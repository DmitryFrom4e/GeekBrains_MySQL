...
-- 8*. Посчитать количество документов у каждого пользователя (doc, docx, html)
-- вариант 1
SELECT
	COUNT(id) AS count_media,
    user_id,
...
FROM media
WHERE filename REGEXP "doc|html|docx" 
GROUP BY user_id
ORDER BY count_media DESC;

-- еще вот так сделал кол-во документов у каждого пользователя (doc, docx, html)
-- вариант 2 - запрос со стороны таблицы пользователей с джойном
select concat(Urs.firstname, " ", Urs.lastname) FullName,
COUNT(MD.id) AS count_media
from users Urs
Left join media MD on Urs.id = MD.user_id
WHERE MD.filename REGEXP "doc|html|docx" 
group by Urs.firstname, Urs.lastname;

-- Посчитать лайки для моих документов (моих медиа)
-- подзапрос к таблице лайков --- жжжесть
SELECT
	COUNT(id) AS count_media,
    user_id,
    (SELECT firstname FROM users WHERE users.id = media.user_id) AS user_name,
    (SELECT lastname FROM users WHERE users.id = media.user_id) AS user_lastname,
    (SELECT email FROM users WHERE users.id = media.user_id) AS user_email,
    (SELECT COUNT(id) FROM likes WHERE likes.media_id = media.id) AS likesCount
FROM media
WHERE filename REGEXP "doc|html|docx" -- не знаю, нужно были ли это условие из предыдущего задания, оставил 
GROUP BY user_id, media.id
ORDER BY count_media DESC;

-- сделал через джойн со стороны таблицы пользователей
-- тут возможен вывод и с нулевым количеством лайков =))
SELECT
	CONCAT(Urs.firstname, " ", Urs.lastname) as FullName,
    email AS user_email,
    COUNT(LKS.id) AS likesCount
FROM users URS 
LEFT JOIN media MD ON URS.id = MD.user_id
LEFT JOIN likes LKS ON LKS.media_id = MD.id
WHERE MD.filename REGEXP "doc|html|docx" -- не знаю, нужно были ли это условие из предыдущего задания, оставил 
GROUP BY Urs.firstname, Urs.lastname, email
