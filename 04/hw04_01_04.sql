DROP DATABASE IF EXISTS homework_4_1;
CREATE DATABASE homework_4_1;
USE homework_4_1;
...
-- 2) не зря же про EXISTS справшивал на семинаре
SELECT shops.shopname
FROM shops
WHERE EXISTS (
    SELECT *
...
-- через date_add с интервалом 1 неделя
SELECT an_name, an_price
FROM Analysis
WHERE an_id IN (
    SELECT ord_an
    ...
    AND ord_datetime < DATE_ADD('2020-02-05 00:00:00', INTERVAL 1 WEEK)
);
