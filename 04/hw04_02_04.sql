DROP DATABASE IF EXISTS homework_4_2;
CREATE DATABASE homework_4_2;
USE homework_4_2;
...
WHERE YEAR(profiles.birthday) > YEAR(CURRENT_DATE()) - 12;
...
JOIN users ON users.id = likes.user_id
JOIN profiles ON profiles.user_id = users.id
WHERE YEAR(profiles.birthday) > YEAR(NOW()) - 12;
...
-- 4. 
SELECT users.id, CONCAT(users.firstname, ' ', users.lastname) AS full_name, COUNT(*) AS total_messages
FROM users
...
JOIN (
    SELECT CASE 
            WHEN initiator_user_id = (SELECT id FROM users WHERE firstname = 'Reuben')
            THEN target_user_id
            ELSE initiator_user_id
        END AS friend_id
    FROM friend_requests
    WHERE friend_requests.`status` = 'approved'
    AND (initiator_user_id = (SELECT id FROM users WHERE firstname = 'Reuben') 
    OR target_user_id = (SELECT id FROM users WHERE firstname = 'Reuben'))
) AS friends 
